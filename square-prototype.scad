...
dimensionB = 3; // [1:1:6]
//in case of square pans give the same value for the both sizes
// first pan size in mm
panSizeA = 26; // [20:1:60]
// second pan size in mm
panSizeB = 48; // [20:1:60]
...
/*[Hidden]*/
panWidth = min(panSizeA,panSizeB);
panHeight = max(panSizeA,panSizeB);
rowCount = dimensionA;
columnCount = dimensionB;
...
magnetOffsetX = 0;
magnetOffsetY = addPushHole ? panHalf / 2 : 0;
pushHoleOffsetX = 0;
pushHoleOffsetY = panHalf / -2;
sphereOffsetY = 17;
sphereOffsetZ = 11;
sphereRadius = 23.5;
...
paletteWidth = (columnCount * panWidth) + ((columnCount - 1) * panSpacing) + (2 * outerMargin);
paletteLength = (rowCount * panHeight) + ((rowCount - 1) * panSpacing) + (2 * outerMargin);
        ...
        difference() {
            ...
            for (r = [0 : rowCount - 1]) {
                for (c = [0 : columnCount - 1]) {
                    xPos = outerMargin + panWidth/2 + c * (panWidth + panSpacing);
                    yPos = outerMargin + panHeight/2 + r * (panHeight + panSpacing);
                    // Pan hole
                    translate([xPos, yPos, paletteThickness - panDepth/2])
                        cube([panWidth,panHeight,panDepth + epsilon],center=true);
                    // Magnet hole
